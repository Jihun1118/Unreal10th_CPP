...
//		AnimInstance->Montage_SetEndDelegate(EndDelegate, CurrentWeaponData->AttackMontage);	// 애니메이션이 끝나는 타이밍에 실행
//
...
void AActionCharacter::OnAttackAction(const FInputActionValue& Value)
{
		...
		//&& CurrentWeapon.IsValid() && CurrentWeapon->CanUse() )
	{
		...
		//{
		//	// 첫번째 콤보 공격
		//	PlayAnimMontage(CurrentWeaponData->AttackMontage);			
		...
		//	IStaminaInterface::Execute_ConsumeStamina(GetStatComponent(), AttackCost);			
		//}
		//else if (AnimInstance->GetCurrentActiveMontage() == CurrentWeaponData->AttackMontage)
		//{
		//	SectionJumpForCombo();			
		//}
	}
